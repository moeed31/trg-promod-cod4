...
init()
{
	...
	registerScoreInfo("plant", 50);
	registerScoreInfo("defuse", 50);
	registerScoreInfo("ninja_defuse", 50);
	registerScoreInfo("capture", 15);
	registerPopupInfo("kill", 5);
	registerPopupInfo("headshot", 5);
	registerPopupInfo("assist", 3);
	registerPopupInfo("suicide", 0);
	registerPopupInfo("teamkill", 0);
	registerPopupInfo("win", 2);
	registerPopupInfo("loss", 1);
	registerPopupInfo("tie", 1.5);
	registerPopupInfo("plant", 3);
	registerPopupInfo("defuse", 3);
	registerPopupInfo("ninja_defuse", 3);
	registerPopupInfo("capture", 3);
	registerPopupInfo("assault", 3);
	registerPopupInfo("assault_assist", 1);
	registerPopupInfo("defend", 3);
	registerPopupInfo("defend_assist", 1);
...
onPlayerConnect()
{
	for(;;)
	{
		...
		// Auto-migrate old VIP levels (4->1, 5->2, 6->3)
		oldVip = player getStat(3253);
		if(oldVip == 4) player setStat(3253, 1);
		else if(oldVip == 5) player setStat(3253, 2);
		else if(oldVip == 6) player setStat(3253, 3);
...
giveRankXP(type, value)
{
	...
	popupValue = value;
	if(isDefined(type) && isDefined(level.popupInfo[type]) && isDefined(level.popupInfo[type]["value"]))
		popupValue = level.popupInfo[type]["value"];

	// always show
	self thread updateRankScoreHUD(popupValue);
	
	if(updateRank())
		self thread updateRankAnnounceHUD();
		
	self checkAutoPrestige();
}
...
registerPopupInfo(type, value)
{
	level.popupInfo[type] = [];
	level.popupInfo[type]["value"] = value;
}
...
{	
	...
	self iprintlnbold("^3YOU HAVE AUTO-PRESTIGED!");
}
...
checkAutoPrestige()
{
	...
	chal1 = self GetStat(3117);
	chal2 = self GetStat(3118);

	if(chal1 != 0 && chal2 != 0)
	{
		if(scripts\utility\challenge_utility::isChallengeCompleted(chal1) && scripts\utility\challenge_utility::isChallengeCompleted(chal2))
		{
			self prestigeUp();
		}
	}
...
{
	awardLevel = self GetStat(3252);
	if(awardLevel == 5 && (num >= 1 && num <= 5))
		return true;
	else if(awardLevel == 4 && (num >= 1 && num <= 4))	
		return true;
	else if(awardLevel == 3 && (num >= 1 && num <= 3))	
		return true;
	else if(awardLevel == 2 && (num >= 1 && num <= 2))	
		return true;
	else if(awardLevel == 1 && (num == 1))		
		return true;
	else 
		return false;
}
...
{
	donationLevel = self GetStat(3253);
	if(donationLevel == 3 && (num >= 1 && num <= 3))
		return true;
	else if(donationLevel == 2 && (num >= 1 && num <= 2))
		return true;
	else if(donationLevel == 1 && num == 1)
		return true;
	else 
		return false;
}
...
{
	awardLevel = self GetStat(3252);
	if(awardLevel == 5 && (num >= 3 && num <= 5))
		return true;
	else if(awardLevel == 4 && (num >= 3 && num <= 4))
		return true;
	else if(awardLevel == 3 && num == 3)
		return true;
	else return false;
}
