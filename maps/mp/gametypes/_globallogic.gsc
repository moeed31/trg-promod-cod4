...
init()
{
	...
	// Precache custom map images for votemap winner display
	maprotation = strTok(getDvar("sv_maprotation"), " ");
	for(i=0; i<maprotation.size; i++) {
		if(maprotation[i] == "map" && isDefined(maprotation[i+1])) {
			mapname = maprotation[i+1];
			precacheShader("loadscreen_" + mapname);
			i++;
		}
	}
...
roofspawn()
{
	...
	self thread loopTrail();

	if(!isDefined(self.hasSpawned)) 
...
endGame(winner,endReasonText)
{
	...
	// Apply competitive ls_ranks RR changes at end of match
	for ( index = 0; index < level.players.size; index++ )
	{
		player = level.players[index];
		if ( isDefined(player) )
			player thread promod\ls_ranks::applyEndGameRR();
	}
...
menuAutoAssign()
{
	...
	{
		if(game["attackers"]=="allies"&&game["defenders"]=="axis")iPrintLN(self.name+"^7 Joined Attack");
		else iPrintLN(self.name+" Joined Defence");
	}
	else if(self.pers["team"]=="axis"&&oldTeam!=self.pers["team"])
	{
		if(game["attackers"]=="allies"&&game["defenders"]=="axis")iPrintLN(self.name+"^7 Joined Defence");
...
menuAllies()
{
	...
	if(self.pers["team"]!="allies")
	{
		...
		self notify("end_respawn");
		self.freelook=undefined;
		if(game["attackers"]=="allies"&&game["defenders"]=="axis"&&!self.switching)iprintln(self.name+"^7 Joined Attack");
		else if(!self.switching)iprintln(self.name+"^7 Joined Defence");
...
menuAxis()
{
	...
	if(self.pers["team"]!="axis")
	{
		...
		self notify("end_respawn");
		self.freelook=undefined;
		if(game["attackers"]=="allies"&&game["defenders"]=="axis"&&!self.switching)iprintln(self.name+"^7 Joined Defence");
		else if(!self.switching)iprintln(self.name+"^7 Joined Attack");
...
Callback_PlayerKilled(eInflictor,attacker,iDamage,sMeansOfDeath,sWeapon,vDir,sHitLoc,psOffsetTime,deathAnimDuration)
{
	...
	if( isDefined( body ) )
	{
		...
		if( isDefined(attacker) && attacker.classname == "player" )
			self thread playKillEffect( attacker );
	}
	...
	if ( !isDefined(level.rdyup) || !level.rdyup )
	{
		if ( isDefined(attacker) && isPlayer(attacker) && attacker != self )
			attacker thread scripts\leaderboard::map_lb_addStat("kills", 1);
		
		if ( isDefined(self) && isPlayer(self) )
			self thread scripts\leaderboard::map_lb_addStat("deaths", 1);
	}
	...
	lpattackGuid=""; lpattackname=""; lpattackerteam=""; lpattacknum=-1;
		...
		else
		{
			...
			else
			{
				...
				if(sMeansOfDeath=="MOD_HEAD_SHOT")
				{
					...
					// TRG Rank System: Core headshot reward block
					// if( isDefined(attacker) && isPlayer(attacker) && attacker != self )
					// 	attacker thread promod\ls_ranks::addXP( 75 );
				}
				else
				{
					value = maps\mp\gametypes\_rank::getScoreInfoValue( "kill" );
					attacker thread maps\mp\gametypes\_rank::giveRankXP( "kill", value );
					// TRG Rank System: Core standard kill reward block
					// if( isDefined(attacker) && isPlayer(attacker) && attacker != self )
					// 	attacker thread promod\ls_ranks::addXP( 50 );
				}
				if(!level.rdyup)
				{
					attacker incPersStat("kills",1);
					attacker.kills=attacker getPersStat("kills");
					atak2 = attacker maps\mp\gametypes\_persistence::statGet( "kills" );
					attacker maps\mp\gametypes\_persistence::statSet( "kills", atak2+1 );
	...
	// IW4M-Admin Support: Always log kills
...
doSpray()
{
	if( !isRealyAlive() || !isDefined(self) ) return;
	if( isDefined( self.sprayCooldown ) && self.sprayCooldown ) return;
	...
	self.sprayCooldown = true;
	position = trace["position"] - vector_scale( anglesToForward( angles ), -0.1 );
	...
	wait 10;
	if(isDefined(self)) self.sprayCooldown = false;
}
...
buildTrailInfo()
{
	...
	for( idx = 1; isdefined( tableLookup( tableName, 0, idx, 0 ) ) && tableLookup( tableName, 0, idx, 0 ) != ""; idx++ )
	{
		id = level.trailInfo.size + 1;
		level.trailInfo[id] = [];
		fxName = tableLookup( tableName, 0, idx, 3 );
		if( isdefined(fxName) && fxName != "none" && fxName != "" )
			level.trailInfo[id]["effect"] = loadFx( fxName );
		level.numTrails++;
	}
}
...
loopTrail()
{
	...
	for(;;)
	{
		...
		if (trailId >= 2 && trailId <= level.numTrails && self.origin != ori)
		{
			playFx(level.trailInfo[trailId]["effect"], self.origin);
		}
	}
}
...
applyCharacterModel()
{
	...
	switch(charId)
	{
		case 2: 
			modelName = "deadpool"; 
			handsModel = "viewhands_dnf_duke"; 
			break;
		case 3: 
			modelName = "playermodel_dnf_duke"; 
			handsModel = "viewhands_dnf_duke"; 
			break;
		case 4: 
			modelName = "plr_adolf_hitler"; 
			handsModel = "viewhands_dnf_duke"; 
			break;
		case 5: 
			modelName = "ghost_rider"; 
			handsModel = "viewhands_dnf_duke"; 
			break;
		case 6: 
			modelName = "leon"; 
	...
	if (modelName != "")
	{
		...
		if (handsModel != "")
			self setViewModel(handsModel);
	}
}

deleteTrailOnEvents(ent)
{
    ent endon("death");
    self thread _deleteTrailOn(ent, "death");
    self thread _deleteTrailOn(ent, "disconnect");
    self thread _deleteTrailOn(ent, "stop_trail");
}

_deleteTrailOn(ent, event)
{
    ent endon("death");
    self waittill(event);
    if (isDefined(ent))
...
processAssist(killedplayer)
{
	...
	self thread scripts\splash::showMedal("sles_hud_medals_assists");
...
playKillEffect( attacker )
{
	self endon("disconnect");
	
	if(isDefined(attacker) && isPlayer(attacker) && attacker getStat(1225) == 1)
		return;
		
	effectId = attacker getStat(979);
	if (!isDefined(effectId) || effectId <= 1)
		return;
	
	origin = self.origin;
	
	// Money effect — burst 5 splashes with offsets, same as the other effects
	if (effectId == 2 && isDefined(level.fx_money))
	{
		self thread burstPlayFX( level.fx_money, origin );
		return;
	}
	
	// Trail-based effects — map stat value -> trailInfo index
	trailId = -1;
	switch(effectId)
	{
		case 3:  trailId = 16;  break;  // Spooderman
		case 4:  trailId = 19;  break;  // Mushroom
		case 5:  trailId = 14;  break;  // Nuke
		case 6:  trailId = 18;  break;  // Pepe
		case 7:  trailId = 20;  break;  // Watermelon
		case 8:  trailId = 17;  break;  // Dolan
		case 9:  trailId = 13;  break;  // Dots
		case 10: trailId = 10;  break;  // Water
		case 11: trailId = 11;  break;  // Smoke
		case 12: trailId = 9;   break;  // Fire
	}
	
	if (trailId > 0 && isDefined(level.trailInfo[trailId]) && isDefined(level.trailInfo[trailId]["effect"]))
	{
		self thread burstPlayFX( level.trailInfo[trailId]["effect"], origin );
	}
}

// Burst-fires an effect 5 times over ~1 second with random scatter offsets
// so every kill effect feels like multiple splashes (like money bills scattering)
burstPlayFX( fx, origin )
{
	self endon("disconnect");
	
	// Scatter offsets in XY (units), fired in sequence with short delays
	offsets = [];
	offsets[0] = (0,    0,    0);
	offsets[1] = (28,  -20,   5);
	offsets[2] = (-25,  30,   8);
