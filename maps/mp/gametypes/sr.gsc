...
onUsePlantObject(player)
{
	...
	if(!self maps\mp\gametypes\_gameobjects::isFriendlyTeam(player.pers["team"]))
	{
		...
		player.pers["plants"]++;
		player thread scripts\splash::showMedal("sles_hud_medals_plants");
...
onUseDefuseObject(player)
{
	...
	player.pers["defuses"]++;
	player thread scripts\splash::showMedal("sles_hud_medals_defuses");
