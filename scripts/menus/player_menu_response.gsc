player(response)
{
	...
	switch(response)
	{
		...
		case "fps":
			...
			fps++;
			if (fps > 1) fps = 0;
			self setstat(1222, fps);
			if (fps == 1) {
				self setClientDvar("r_fullbright", 1);
				self setClientDvar("ui_fullbright_text", "^2ON");
			} else {
				self setClientDvar("r_fullbright", 0);
				self setClientDvar("ui_fullbright_text", "^1OFF");
			}
			break;
			...
			fov++;
			if (fov > 7) fov = 2; // 2 to 7 are valid
			self setstat(1322, fov);
			
			fovscale = 1.0;
			fov_text = "";
			switch(fov) {
				case 2: fovscale = 1.0; fov_text = "1"; break;
				case 3: fovscale = 1.125; fov_text = "^21.125^7"; break;
				case 4: fovscale = 1.25; fov_text = "1.25"; break;
				case 5: fovscale = 1.30; fov_text = "1.30"; break;
				case 6: fovscale = 1.40; fov_text = "1.40"; break;
				case 7: fovscale = 1.50; fov_text = "1.50"; break;
			}
			self setClientDvar("ui_fov_text", fov_text);
			self setClientDvar("ui_fov_scale", fov);
			self setClientDvar("cg_fovscale", fovscale);
			break;


		case "maxfps":
			mfps = self getstat(1422);
			mfps++;
			if (mfps > 4) mfps = 0;
			self setstat(1422, mfps);
			
			fpsval = 250;
			switch(mfps) {
				case 0: fpsval = 100; break;
				case 1: fpsval = 125; break;
				case 2: fpsval = 175; break;
				case 3: fpsval = 200; break;
				case 4: fpsval = 250; break;
			}
			self setClientDvar("com_maxfps", fpsval);
			break;
		
		// FOV
		case "fov1":
			self setstat(1322, 2);
			self setClientDvar("ui_fov_scale", 2);
			self setClientDvar( "cg_fovscale", 1);
			break;
			
		case "fov2":
			self setstat(1322, 3);
			self setClientDvar("ui_fov_scale", 3);
			self setClientDvar( "cg_fovscale", 1.1);
			break;

		case "fov3":
			self setstat(1322, 4);
			self setClientDvar("ui_fov_scale", 4);
			self setClientDvar( "cg_fovscale", 1.125);
			break;

		case "fov4":
			self setstat(1322, 5);
			self setClientDvar("ui_fov_scale", 5);
			self setClientDvar( "cg_fovscale", 1.20);
			break;

		case "fov5":
			self setstat(1322, 6);
			self setClientDvar("ui_fov_scale", 6);
			self setClientDvar( "cg_fovscale", 1.25);
			break;

		case "fov6":
			self setstat(1322, 7);
			self setClientDvar("ui_fov_scale", 7);
			self setClientDvar( "cg_fovscale", 1.30);
			break;

		case "fov7":
			self setstat(1322, 8);
			self setClientDvar("ui_fov_scale", 8);
			self setClientDvar( "cg_fovscale", 1.35);
			break;

		case "fov8":
			self setstat(1322, 9);
			self setClientDvar("ui_fov_scale", 9);
			self setClientDvar( "cg_fovscale", 1.40);
			break;

		case "fov9":
			self setstat(1322, 10);
			self setClientDvar("ui_fov_scale", 10);
		...
		case "endround_music_toggle":
			if(self getstat(1223) == 2)
				self setstat(1223, 1);
			else
				self setstat(1223, 2);
			break;
			
		case "killeffects_toggle":
			if(self getstat(1225) == 1)
				self setstat(1225, 0);
			else
				self setstat(1225, 1);
			break;
